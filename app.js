...
(function () {
  ...
  function defaultState() {
    return {
      settings: { kcal: 700, ratio: 1.8, mahlzeiten: 5, eiweiss: 20, weight: 8, proteinPerKg: 1.5, showOhneKeto: true, showMitKeto: false, filter: "alle", onlyQuelle: false, sort: "kategorie" },
  ...
  function load() {
    try {
      ...
      // Migration: alte 3-fach-Auswahl "ketocal" -> zwei Schalter
      if (p.settings && p.settings.ketocal && p.settings.showOhneKeto === undefined && p.settings.showMitKeto === undefined) {
        settings.showOhneKeto = p.settings.ketocal !== "mit";
        settings.showMitKeto = p.settings.ketocal !== "ohne";
      }
      return {
        settings: settings,
  ...
  function renderRezepte() {
    ...
    // KetoCal als zwei unabhängige Schalter; keiner aktiv = alle zeigen
    const wantOhne = !!s.showOhneKeto, wantMit = !!s.showMitKeto;
    const ketoAny = wantOhne || wantMit;
    const showOhne = !ketoAny || wantOhne;
    const showMit = !ketoAny || wantMit;
    const warn = showOhne; // Hinweis nur wenn Rezepte ohne KetoCal sichtbar sind
    $("permeal").innerHTML =
    ...
    const recipes = allRecipes()
      .filter(r => (r.ketocal ? showMit : showOhne))
    ...
    $("recipe-count").textContent =
      ...
      (showOhne && !showMit ? " ohne KetoCal" : showMit && !showOhne ? " mit KetoCal" : "");
